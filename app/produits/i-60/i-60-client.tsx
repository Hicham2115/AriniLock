...
import i60img8 from "./assets/6.png";
...
const FEATURE_META = [
  { img: i60img9, icon: ScanFace },
  { img: i60img8, icon: Camera },
  { img: i60img6, icon: AlertTriangle },
] as const;
const STATS_META = [
...
function Img({
  ...
  return (
    <div
      className={cn("relative overflow-hidden rounded-2xl bg-white", className)}
    >
      <Image
        src={src}
        alt={label}
  ...
  return (
    <>
      ...
      <main className="bg-white">
        ...
        {/* ══ PAGE 2 — FONCTIONNALITÉS, STATS, INSTALLATION & SPECS (condensé) ══ */}
        <section
          id="fonctionnalites"
              ...
              {FEATURES.map((f, i) => (
                <Reveal key={f.tag} delay={i * 0.06}>
                  <div className="flex h-full flex-col rounded-2xl border border-gray-100 bg-white p-5">
                    ...
                    <div className="mt-3 inline-flex w-fit items-center gap-2 rounded-full border border-primary/15 bg-primary/5 px-3 py-1.5 text-[10px] font-semibold text-primary">
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-3">
                {STEPS.map((step, i) => (
                  <Reveal key={step.num} delay={i * 0.06}>
                    <div className="flex h-full gap-4 rounded-2xl border border-gray-100 bg-gray-50/60 p-5">
                      <span
                        className="font-display2 shrink-0 leading-none text-primary/25"
